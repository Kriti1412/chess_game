...
public class Board
{
  ...
  Piece typeOfPiece(int xi,int yi)
  {
	return bo[xi][yi].p1;  
  }
  ...
  boolean checkPlayer(String color,int xi,int yi,int xc,int yc)
  {
	 boolean ch = false;
	 if(color == "black")                                                  // FOR BLACK COLOUR.....
	 {
	   if(bo[y][z].p1.validMoveOfPiece(y,z,xc,yc) &&validMoveOfGame(bo[y][z].p1,y,z,xc,yc))  // FOR PIECE ALREADY GIVING CHECK.....
	   {
		   return true;
	   }
	   
       else if((xc-xi)==0)                                                      // IF KING AND PIECE WERE IN SAME ROW.....
	   {
		 for(int i=0;i<8;i++)
		 {
			 if(bo[xc][i].p1==null)
  ...
  boolean checkOpponent(String color,int xc,int yc)
  {
	  boolean ch = false;
	  for(int i=0;i<8;i++)
	  {
	    for(int j=0;j<8;j++)
		  {
			 ...
			 else if(bo[i][j].p1.colorpiece==color)
			 {
				if(bo[i][j].p1.validMoveOfPiece(i,j,xc,yc) && validMoveOfGame(bo[i][j].p1,i,j,xc,yc))
				{
					  y = i;
					  z = j;
                      return true;
